...
public class SwipeDismissListView extends ListView {
        ...
        if (isHorizontalScroll) {
            ...
            if (ev.getAction() == MotionEvent.ACTION_UP || ev.getAction() == MotionEvent.ACTION_CANCEL) {
                ...
                if (Math.abs(x - downX) < getWidth() * DISMISS_FACTOR) {
                    //还原
                    currentView.startAnimation(new XTranslationAnim(currentView, 0, null));

                } else {
                    //dismiss
                    if (x - downX >= 0) {
                        //向右
                        currentView.startAnimation(new XTranslationAnim(currentView, getWidth(), new Animation.AnimationListener() {
                            @Override
                            ...
                            public void onAnimationEnd(Animation animation) {
                                shorten(currentView, currentIndex);
                            }
    ...
    private class XTranslationAnim extends Animation {

        private View v;
        private float src, desc;

        public XTranslationAnim(View v, float desc, AnimationListener listener) {
            setAnimationListener(listener);
            this.v = v;
            this.desc = desc;
            this.src = v.getTranslationX();
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            v.setTranslationX(src + (desc - src) * interpolatedTime);
    ...
    private void shorten(final View v, final int index){
        v.startAnimation(new HeightAnim(v, 1, new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                ...
                v.setTranslationX(0);
                if(listener != null){
                    listener.onDismiss(index + getFirstVisiblePosition());
                }
            }
